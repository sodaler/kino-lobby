<div class="movie-info border-b border-gray-800">
    ...
    <div class="movie-images" x-data="{ isOpen: false, image: '' }">
        ...
        <div class="movie-cast border-b border-gray-800">
            <div class="container mx-auto px-4 py-16">
                <h2 class="text-4xl font-semibold">Похожие фильмы</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                    @foreach($similarMovies as $similarMovie)
                        @if(!empty($similarMovie['nameRu']))
                            <div class="mt-8">
                                <a href="{{ route('movies.show', $similarMovie['filmId']) }}">
                                    <img src="{{ $similarMovie['posterUrl'] }}" alt="parasite"
                                         class="hover:opacity-75 transition ease-in-out duration-150 object-contain w-96 h-96">
                                </a>
                                <div class="mt-2">
                                    <a href="{{ route('movies.show', $similarMovie['filmId']) }}" class="text-lg mt-2 hover:text-gray-300">
                                        {{ $similarMovie['nameRu'] }}
                                    </a>
                                </div>
                            </div>
